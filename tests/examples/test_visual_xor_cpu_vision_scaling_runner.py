...
def test_build_benchmark_matrix_returns_deterministic_cartesian_product():
    matrix = scaling.build_benchmark_matrix(
        [1, 2],
        [2, 4],
        [8, 16],
    )
...
def test_build_benchmark_matrix_rejects_invalid_dimensions(
    ...
    with pytest.raises(ValueError, match=error_pattern):
        scaling.build_benchmark_matrix(replica_counts, thread_counts, batch_sizes)
...
def test_run_scaling_matrix_measures_each_configuration_once_and_applies_capacity_gate():
    ...
    summary = scaling.run_scaling_matrix(
        matrix,
...
def test_parse_args_accepts_required_paths_and_comma_separated_matrix_dimensions():
    args = scaling.parse_args(_complete_cli_argv())
...
def test_main_builds_and_measures_the_requested_matrix_with_one_factory(tmp_path):
    ...
    artifact = scaling.main(argv, measurement_factory=measurement_factory)
    ...
    assert measured_configurations == scaling.build_benchmark_matrix([1, 2], [4], [8, 16])
...
def test_module_execution_delegates_to_main():
    module = ast.parse(Path(scaling.__file__).read_text())
    main_guards = [
        statement
...
def test_benchmark_module_does_not_import_model_or_dataset_dependencies_eagerly(monkeypatch):
    module_path = Path(scaling.__file__)
    spec = importlib.util.spec_from_file_location("_measure_cpu_vision_scaling_import_test", module_path)
