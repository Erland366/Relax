...
def test_analyze_cache_ablation_pairs_steady_cycles_and_separates_tiers(tmp_path):
    from examples.visual_xor.analyze_cpu_vision_cache import analyze_cache_ablation, write_cache_artifact

    tier1_log = tmp_path / "tier1.log"
    tier2_log = tmp_path / "tier2.log"
    ...
    result = analyze_cache_ablation(
        {"tier1_only": tier1_log, "tier1_tier2": tier2_log},
        first_steady_cycle=2,
        expected_steady_cycles=2,
    )

    assert result["profiles"]["tier1_only"]["producer_cache_hit_rate"] == 1.0
    assert result["profiles"]["tier1_tier2"]["sglang_id_hit_rate"] == 1.0
    assert result["request_body_byte_reduction"] == pytest.approx(1 - 64_000 / 22_400_000)
    assert result["rollout_wall_reduction"] == pytest.approx(1 - 2.8 / 3.0)
    ...
    artifact = write_cache_artifact(output_path, result)
    assert artifact["schema_version"] == 1
    assert json.loads(output_path.read_text()) == artifact


def test_analyze_cache_ablation_rejects_missing_rollout_cycle(tmp_path):
    from examples.visual_xor.analyze_cpu_vision_cache import analyze_cache_ablation
    ...
    with pytest.raises(ValueError, match="paired steady cycles"):
        analyze_cache_ablation(
            {"tier1_only": log_path, "tier1_tier2": log_path},
            first_steady_cycle=2,
            expected_steady_cycles=2,
        )
