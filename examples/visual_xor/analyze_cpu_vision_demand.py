...
"""Analyze paired CPU-vision and actor-cycle metrics from E1 console logs."""
...
def _parse_metrics_log(log_path: str | Path) -> tuple[dict[int, dict], dict[int, dict]]:
    path = Path(log_path)
    if not path.is_file():
        raise FileNotFoundError(f"E1 console log does not exist: {path}")
    ...
    for raw_line in path.read_text(errors="replace").splitlines():
        ...
        if vision_match:
            ...
            continue
        actor_match = _ACTOR_METRICS.search(line)
        if actor_match:
            actor_cycles[int(actor_match.group(1))] = ast.literal_eval(actor_match.group(2))
    return vision_cycles, actor_cycles
...
def _analyze_profile(
    ...
    *,
    ...
    vision_cycles, actor_cycles = _parse_metrics_log(log_path)
    expected_cycles = list(range(first_steady_cycle, first_steady_cycle + expected_steady_cycles))
    paired_cycles = [
        cycle for cycle in expected_cycles if cycle in vision_cycles and cycle in actor_cycles
    ]
    if paired_cycles != expected_cycles:
        missing_vision = sorted(set(expected_cycles) - vision_cycles.keys())
        missing_actor = sorted(set(expected_cycles) - actor_cycles.keys())
        raise ValueError(
            f"E1 log {log_path} does not contain all paired steady cycles; "
            f"missing_vision={missing_vision}, missing_actor={missing_actor}"
        )
    ...
    for cycle in expected_cycles:
        ...
        step_time = float(actor["perf/step_time"])
        wait_ratio = float(actor["perf/wait_time_ratio"])
        if unique_images <= 0 or requests <= 0 or encoded_images <= 0:
            raise ValueError(f"E1 cycle {cycle} in {log_path} contains non-positive image/request counts")
        if not math.isfinite(step_time) or step_time <= 0:
            raise ValueError(f"E1 cycle {cycle} in {log_path} contains invalid actor step time {step_time}")
        if not math.isfinite(wait_ratio) or wait_ratio < 0:
            raise ValueError(f"E1 cycle {cycle} in {log_path} contains invalid actor wait ratio {wait_ratio}")
        cycle_results.append(
            {
                ...
                "actor_step_time_seconds": step_time,
                "actor_wait_ratio": wait_ratio,
    ...
    return {
        ...
        "mean_actor_wait_ratio": sum(cycle["actor_wait_ratio"] for cycle in cycle_results)
        / len(cycle_results),
        "max_actor_wait_ratio": max(cycle["actor_wait_ratio"] for cycle in cycle_results),
...
def analyze_e1_demand(
    profile_logs: Mapping[str, str | Path],
    *,
    first_steady_cycle: int = 2,
    expected_steady_cycles: int = 10,
    saturation_profile: str = "D2",
    actor_wait_threshold: float = 0.05,
) -> dict[str, object]:
    """Compute E1 consumer demand without substituting backend producer throughput."""
    if not profile_logs:
    ...
    if saturation_profile not in profile_logs:
        raise ValueError(f"saturation profile {saturation_profile!r} is absent from profile_logs")
    ...
    saturation_result = profiles[saturation_profile]
    return {
        "first_steady_cycle": first_steady_cycle,
        "expected_steady_cycles": expected_steady_cycles,
        "actor_wait_threshold": actor_wait_threshold,
        "saturation_profile": saturation_profile,
        "saturation_profile_reached_actor_wait_gate": saturation_result["max_actor_wait_ratio"]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--saturation-profile", default="D2")
