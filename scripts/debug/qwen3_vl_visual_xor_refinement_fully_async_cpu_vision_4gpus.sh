...
export SKIP_GPU_VISION_ENCODER="${SKIP_GPU_VISION_ENCODER:-0}"
...
export VISION_DEVICE_MODE="${VISION_DEVICE_MODE:-fixed}"
export VISION_DEVICE_PLAN="${VISION_DEVICE_PLAN:-}"
export VISION_DEVICE_MINIMUM_GAP="${VISION_DEVICE_MINIMUM_GAP:-}"
...
if [ "${CPU_VISION_CAPACITY_MODE:-0}" = "1" ]; then
    "${PYTHON:-python}" -m examples.visual_xor.cpu_vision_capacity_preflight \
        --vision-encoder-num-replicas "${VISION_ENCODER_NUM_REPLICAS}" \
        --vision-encoder-num-cpus "${VISION_ENCODER_NUM_CPUS}"
fi
if [ "${VISION_DEVICE_MODE}" = "automatic" ]; then
    CPU_VISION_MODE=automatic
elif [ "${SKIP_GPU_VISION_ENCODER}" = "1" ]; then
    CPU_VISION_MODE=cpu-skip-gpu-encoder
else
    CPU_VISION_MODE=cpu
fi
...
export RUN_LOG="${RUN_LOG:-${ROOT_DIR}/log/visual-xor-refinement-${CPU_VISION_MODE}-${RUN_TAG}.log}"
