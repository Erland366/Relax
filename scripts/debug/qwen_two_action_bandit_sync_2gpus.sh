...
: "${HF_CHECKPOINT:?Set HF_CHECKPOINT to the accepted A/B SFT model directory}"
...
export ASSET_DIR="${ASSET_DIR:-${ROOT_DIR}/relax_assets}"
export CONDA_ENV_NAME="${CONDA_ENV_NAME:-relaxrl_rocm_after_fix}"

export MODEL_CONFIG_NAME=qwen3-mock
export MODEL_ASSET_NAME=Qwen3-Mock-0.5B-Two-Action-Bandit-SFT
export MODEL_LOG_NAME=qwen3-mock-0.5b-two-action-bandit-sft
export PROMPT_SET="${ROOT_DIR}/examples/two_action_bandit/prompts.jsonl"
export APPLY_CHAT_TEMPLATE_KWARGS='{"enable_thinking": false}'

export RELAX_EXECUTION_MODE=sync
export USE_COLLOCATE=0
export RM_TYPE=two_action_bandit
...
export RUN_LOG="${ROOT_DIR}/log/two-action-bandit-${RUN_TAG}.log"
export WANDB_MODE="${WANDB_MODE:-online}"
export WANDB_ENTITY="${WANDB_ENTITY:-}"
export WANDB_PROJECT="${WANDB_PROJECT:-relax-amd-two-action-bandit}"
export WANDB_GROUP="${WANDB_GROUP:-two-action-bandit-${RUN_TAG}}"
