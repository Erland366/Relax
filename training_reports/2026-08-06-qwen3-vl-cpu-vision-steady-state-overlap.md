...
- **Date:** 2026-08-06
- **Status:** passed matched 20-rollout fully asynchronous CPU-omitted and
  native-GPU performance runs; native grouped sampling remains a qualified
...
The CPU path made each rollout slower than native GPU vision, but removed the
...
reduction in complete steady actor-cycle time, and a 738.03 MiB reduction at
...
than the native raw-image request, so transport remains inefficient, but it is
...
|---|---|---|---|---|
| CPU omitted, no cache | `raysubmit_fahWwVRtsD5QD6An` | `drbdd32z` | `log/visual-xor-refinement-cpu-omitted-20260806_081340.log` | `benchmark_results/cpu_vision/20260806_overlap_cpu_omitted_nocache/` |
| Native GPU, grouped | `raysubmit_Eu9kS3dHqyZJ37rX` | `k936t7l6` | `log/visual-xor-refinement-native-gpu-20260806_083125.log` | `benchmark_results/cpu_vision/20260806_overlap_native_grouped/` |
...
## Native grouped correctness qualification

Native raw-image generation was extended through the same narrow fresh-group
...
The probe returned the same generated token and text for all 64 native grouped
branches, and the ordinary scalar native-versus-packed comparison still
passed with a maximum action-log-probability delta of `1.4305e-6`. However,
native grouped versus native scalar exceeded the strict `0.01` branch-score
gates:

| Native grouped gate | Observed | Limit |
|---|---:|---:|
...
The gate therefore correctly remained failed. The native run is a matched
throughput and memory counterfactual with token-level agreement, not strict
evidence that stochastic `n=8` native branching reproduces scalar native
...
| Timeline metric | CPU omitted | Native grouped |
|---|---:|---:|
...
| Rollout wall overlapping actor work | 105 s / 100% | 79 s / 100% |
...
actor. All 20 CPU rollouts were generated by 08:27:25 UTC while actor cycle 15
was still running.

The native producer showed the same scheduling relationship. Its steady
rollout averaged 3.263 seconds, filled the same staleness window, and generated
all 20 rollouts while actor cycle 15 was still running. In both modes the
...
| Metric | CPU omitted mean | Native grouped mean | CPU change |
|---|---:|---:|---:|
...
| Complete actor cycle | 26.829 s | 29.700 s | 9.67% lower |
| Actor wait ratio | 0.880% | 0.786% | both negligible |
| Steady rollout wall | 4.789 s | 3.263 s | 46.77% higher |

CPU rollout is slower, but it remains about 5.6 times shorter than the CPU
actor cycle. Offloading the frozen visual computation from the actor path more
than compensates for the slower producer. The 2.14-second actor-compute
reduction is consistent with skipping the frozen visual tower, while the
0.75-second weight-update reduction is consistent with the smaller omitted
...
native. CPU service startup delayed rollout 0 by about 16 seconds relative to
native, but the faster steady drain recovered that cost and finished the job
44.501 seconds earlier. The two stochastic trajectories were comparable but
not identical: mean rollout reward was 0.7203 CPU versus 0.7188 native, valid
action rate was 1.0 in both, and response length was exactly two tokens in
both.

## Transport

| Per 64-sample rollout | CPU omitted | Native grouped |
|---|---:|---:|
...
Packed CPU features therefore used about 383.5 times the native request bytes,
...
| Role/device | CPU omitted | Native grouped | CPU reduction |
|---|---:|---:|---:|
...
deltas combine omitted frozen parameters with changes in temporary visual
activations. Peak sampling is process-level evidence, not a precise allocator
decomposition.

## Decision

Keep the current stateless packed CPU path and use omission mode for the next
...
until actor wait rises or the CPU producer no longer reaches staleness
...
half of each steady actor cycle and is now a larger optimization target than
CPU vision transport. Before using grouped native generation as a correctness
