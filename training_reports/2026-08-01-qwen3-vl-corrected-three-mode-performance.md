# Qwen3-VL Corrected Three-Mode Performance Boundary

- **Date:** 2026-08-01
- **Status:** matched native, CPU-resident, and CPU-omitted runs passed;
...
Repeat the native GPU, CPU-precomputed with resident GPU visual weights, and
CPU-precomputed with omitted GPU visual weights comparison after correcting
...
This supersedes the performance interpretation of the 2026-07-31 three-mode
...
and maximum encoder batch size eight. The native mode froze the GPU vision
...
|---|---|---|---:|---:|
| Native GPU | `raysubmit_Gr5FC3WM75vx2MHc` | `z695pdde` | 0 | 704.4 s |
| CPU resident | `raysubmit_4EiVbt6dG5Dk3EMn` | `w7n3s64f` | 0 | 1024.7 s |
| CPU omitted | `raysubmit_dC75CREAA7syfy5a` | `dlwserrn` | 0 | 1047.1 s |

The first native monitor attempt used an unrelated inherited virtual
environment and failed before Ray startup. Its `native_gpu_vram.json` artifact
is preserved but excluded. `native_gpu_vram_retry1.json` is the successful
...
|---|---:|---:|---:|---:|---:|
| Native GPU | 0.7090 | 1.0 | 0.4785 | 0.5234 | 0.5000 |
| CPU resident | 0.6816 | 1.0 | 0.4785 | 0.5234 | 0.5000 |
| CPU omitted | 0.7031 | 1.0 | 0.4531 | 0.5234 | 0.5000 |

These stochastic evaluations are consistent with the deterministic live
SGLang parity gate: both corrected CPU modes recover native-like task
behavior, and GPU-weight omission does not change the representation. The
...
|---|---:|---:|---:|---:|---:|
| Native GPU | 2.944 GiB | 2.511 GiB | 7.778 GiB | 7.257 GiB | 20.155 GiB |
| CPU resident | 2.899 GiB | 2.443 GiB | 7.262 GiB | 7.228 GiB | 19.093 GiB |
| CPU omitted | 2.840 GiB | 2.418 GiB | 7.215 GiB | 7.173 GiB | 19.279 GiB |

CPU omitted reduced every per-device peak relative to CPU resident:

| Device | Resident minus omitted |
|---|---:|
...
The simultaneous total did not isolate the omission effect in this fully
asynchronous repeat. Its peak was 190.18 MiB higher in the omitted run even
...
similar per-device omission reductions. Therefore:

- per-device omission savings are repeatable and role-local;
- a single fully asynchronous simultaneous maximum is schedule-sensitive;
  and
- a stronger cluster-level omission number requires repeated runs or a
  synchronized phase-specific memory probe, not selection of the lower of two
  asynchronous samples.

Native-to-CPU comparisons include execution-path and allocation-timing
changes, not only visual weights. They must not be labeled as omitted-weight
savings.
...
| Metric | Native GPU | CPU resident | CPU omitted |
|---|---:|---:|---:|
...
Relative to native evaluation, CPU request bodies increased from 1.40 MB to
...
| Mode | Wall time | Relative to native |
|---|---:|---:|
| Native GPU | 704.4 s | 1.000x |
| CPU resident | 1024.7 s | 1.455x |
| CPU omitted | 1047.1 s | 1.487x |

Omission changed resident CPU-mode wall time by only 2.2%. This confirms that
omission is a memory feature, not a throughput optimization. The corrected
CPU modes remain about 46-49% slower than native under this workload.
...
6. rerun this matched three-mode workload before testing more CPU threads or
   replicas.
...
- Native GPU: `log/visual-xor-refinement-native-gpu-20260801_141157.log`
- CPU resident: `log/visual-xor-refinement-cpu-resident-20260801_142408.log`
- CPU omitted: `log/visual-xor-refinement-cpu-omitted-20260801_144228.log`
...
- Native GPU: `https://wandb.ai/erlandpg/relax-amd-visual-xor-refinement/runs/z695pdde`
- CPU resident: `https://wandb.ai/erlandpg/relax-amd-visual-xor-refinement/runs/w7n3s64f`
- CPU omitted: `https://wandb.ai/erlandpg/relax-amd-visual-xor-refinement/runs/dlwserrn`
